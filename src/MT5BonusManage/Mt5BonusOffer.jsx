...
const [edit,setEdit]=useState({
    ...
    const columns = [
        ...
        },
        {
            name: "Start Date",
            selector: (row) => {
                return <span title={row.start_date}>{row.start_date}</span>;
            },
            sortable: true,
            reorder: true,
            grow: 1,
        ...
        },
        {
            ...
            selector: (row) => {
                return <span title={row.bonus_percentage}>{row.bonus_percentage}%</span>;
            }, sortable: true,
            reorder: true,
            grow: 0.5,
            wrap: true,
        ...
        },
        {
            ...
            selector: (row) => {
                return <div className="d-flex"><Button
                    ...
                    onClick={() => { actionMenuPopup("reject", row.id) }}
                >
...
const updateoffer=(prop)=>{
    ...
    setSelectedBonusOfferImage(undefined)
    console.log("start_date",`${start_date.getFullYear()}-${(start_date.getMonth().toString()).length == "1" ?0:""}${start_date.getMonth()}-${(start_date.getDate().toString()).length == "1" ?0:""}${start_date.getDate()}`)
    setEdit({
        id:prop.id,
    bonus_title:prop.bonus_title, 
    bonus_percentage:prop.bonus_percentage,
    start_date:`${start_date.getFullYear()}-${start_date.getMonth().toString.length == 1 ?0:""}${start_date.getMonth()}-${start_date.getDate().toString().length == 1 ?0:""}${start_date.getDate()}`,
    end_date:`${end_date.getFullYear()}-${end_date.getMonth().toString.length == 1 ?0:""}${end_date.getMonth()}-${end_date.getDate().toString().length == 1 ?0:""}${end_date.getDate()}`,
...
const onUpdate=async()=>{
    ...
    }else if(selectedBonusOfferImage==undefined && edit.bonus_offer_image==""){
    ...
    const manageDialogActionButton = () => {
        if(dialogTitle=="Update Bonus Offer" || dialogTitle=="Add Bonus Offer"){
            return(
    ...
    const manageContent = () => {
        ...
        <Grid item md={6}>     <FormControl variant="standard" sx={{ width: "100%" }}>
                  <InputLabel id="demo-simple-select-standard-label" >
                    Status
                  </InputLabel>
                  <Select label="Status" name="status" onChange={input} value={edit.status} focused >
        ...
        <label>Offer Image :</label>
                ...
                >
                  <Input
                    ...
                    type="file"
                    onChange={(e) => onSelectFile(e, "bonus_offer_image")}
                  />
                  {selectedBonusOfferImage ? (
                    <img
