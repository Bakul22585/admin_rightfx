...
const Link = () => {
    ...
    const column = [
        ...
        },
        {
            name: 'STATUS',
            selector: (row) => {
                return <span title={(row.status == "1") ? "Enable" : "Disable"}>{(row.status == "1") ? "Enable" : "Disable"}</span>;
            },
            sortable: true,
            reorder: true,
            grow: 0.3,
        ...
        },
        {
            name: 'FULL URL',
            selector: (row) => {
                return <div className='full-url-section'>
                    <a className='linkColor' title={row.campaign_url} href={row.campaign_url} target='_blank'>{row.campaign_url}</a>
                    <button className="copy_link" onClick={(e) => {
                        navigator.clipboard.writeText(row.campaign_url).then(function () {
                            ...
                            <Select
                                ...
                                onChange={input}>
                                {
                                    sourceMasterList.list_salesman?.map((item) => {
                                        return (
                                            <MenuItem value={item.sales_person}>{item.sales_person_name}</MenuItem>
                                        )
                                    })
                                }
                            </Select>
                                    ...
                                    <CardContent className="py-3">
                                        <Grid container spacing={2}>
                                            <Grid item sm={12} md={12} lg={12}>
                                                <CommonTable url={`${Url}/datatable/campaign_list.php`} column={column} sort='1' search={searchBy} searchWord={searchKeyword} param={param} refresh={refresh} />
                                            </Grid>
                                        </Grid>
                                    </CardContent>
                                </Paper>
